...
Node::Node(ne::NodeId id, std::vector<std::shared_ptr<Pin>> input_pins,
           std::vector<std::shared_ptr<Pin>> output_pins, std::string name, ImColor color)
    : id_{id},
...
auto Node::GetInputPins() -> std::vector<std::shared_ptr<Pin>>& { return input_pins_; }

auto Node::GetOutputPins() const -> const std::vector<std::shared_ptr<Pin>>& {
  return output_pins_;
}

auto Node::GetOutputPins() -> std::vector<std::shared_ptr<Pin>>& { return output_pins_; }