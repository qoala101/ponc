...
// ---
void FlowCalculator::Recalculate(core::Project &project) {
  const auto &diagram = project.GetDiagram();
  RebuildFlowTree(diagram.GetNodes(), diagram.GetLinks());
...
void FlowCalculator::RebuildFlowTree(
    const std::vector<std::shared_ptr<core::INode>> &nodes,
    const std::vector<core::Link> &links) {
  flow_tree_.root_nodes = FindRootNodes(nodes, links);
